...
function parseWeappLogTimestamp(line) {
  const match = String(line ?? '').match(
...
export function filterWeappLogSince(value, startedAt, lineLimit = 160) {
  if (!(startedAt instanceof Date) || Number.isNaN(startedAt.valueOf())) {
    return summarizeTextBlock(value, lineLimit);
  }
  ...
  const filtered = lines.filter((line) => {
    const timestamp = parseWeappLogTimestamp(line);
    return timestamp && timestamp >= startedAt;
  });
...
export function detectDevtoolsCompileCycle(value) {
  const text = String(value ?? '');
  const hasRestartCompile = /restart appservice compile/iu.test(text);
  const hasAppserviceReload = /appservice reload/iu.test(text);
  const hasPageReady = /webview page ready/iu.test(text);
  const completed = hasRestartCompile && hasPageReady;
